...
BACKUP_DIR="/tmp/netmon-full-backup-$(date +%Y%m%d_%H%M%S)"
...
cp /etc/systemd/system/netmon.service $BACKUP_DIR/ 2>/dev/null || echo "没有单用户服务文件需要备份"
cp /etc/systemd/system/netmon-multi.service $BACKUP_DIR/ 2>/dev/null || echo "没有多用户服务文件需要备份"
echo "✅ 配置已备份到: $BACKUP_DIR"

echo ""
echo "🗂️ 创建新目录结构..."
mkdir -p $INSTALL_DIR/{core,single,multi/api,venv}

echo ""
echo "📥 下载最新代码..."
cd /tmp
rm -rf netmon-full-install
git clone $REPO_URL netmon-full-install
cd netmon-full-install
...
echo ""
echo "🔄 启动 NetMon 服务..."
systemctl enable netmon netmon-multi
systemctl start netmon netmon-multi
...
echo "   单用户版: http://localhost:5000"
echo "   多用户版: http://localhost:80"
echo "   单用户服务管理: sudo systemctl [start|stop|restart|status] netmon"
echo "   多用户服务管理: sudo systemctl [start|stop|restart|status] netmon-multi"
echo ""
